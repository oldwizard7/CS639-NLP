...
OFFLINE_ANALYSIS = os.getenv("THOUGHT_ANCHORS_OFFLINE") == "1" or os.getenv(
    "HF_HUB_OFFLINE"
) == "1"
...
def problem_identifier(problem_dir: Path) -> str:
    """Return a stable id for MATH and MMLU problem directories."""
    name = problem_dir.name
    return name[len("problem_") :] if name.startswith("problem_") else name


def problem_matches_filter(problem_dir: Path, requested: set[str]) -> bool:
    identifier = problem_identifier(problem_dir)
    suffix = identifier.rsplit("_", 1)[-1]
    return (
        identifier in requested
        or problem_dir.name in requested
...
# Set up OpenAI API key. Numeric Thought Anchors metrics do not require this;
# when it is absent we keep labels/summaries heuristic instead of failing.
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
client = OpenAI(api_key=OPENAI_API_KEY) if OPENAI_API_KEY else None
if client is None:
    print("OPENAI_API_KEY not found; skipping LLM labels/summaries.")
...
def get_tokenizer():
    """Lazily load the tokenizer; fall back to approximate counts offline."""
    global tokenizer, tokenizer_load_failed
    if tokenizer is not None or tokenizer_load_failed:
        return tokenizer
    if AutoTokenizer is None:
        tokenizer_load_failed = True
        print("transformers is unavailable; using approximate token counts.")
        return None
    try:
        ...
        tokenizer_load_failed = True
        print(f"Could not load tokenizer ({exc}); using approximate token counts.")
        return None
    return tokenizer
...
def count_tokens(text: str, approximate: bool = True) -> int:
    ...
    if loaded_tokenizer is None:
        return len(text) // 4
    return len(loaded_tokenizer.encode(text))
...
def generate_chunk_summary(chunk_text: str) -> str:
    ...
    if client is None:
        ...
        return " ".join(words[:4]).lower() if words else "unknown action"
...
def generate_problem_nickname(problem_text: str) -> str:
    ...
    """
    if client is None:
        words = re.sub(r"\s+", " ", problem_text).strip().split()
        return " ".join(words[:4]) if words else "math problem"
...
def lexical_similarity(text_a: str, text_b: str) -> float:
    """Cheap fallback similarity for offline analysis when embeddings are unavailable."""
...
def chunk_similarity(
    ...
    """Use embedding cosine similarity when available, otherwise lexical overlap."""
    if removed in chunk_embedding_cache and resampled in chunk_embedding_cache:
        removed_embedding = chunk_embedding_cache[removed]
        resampled_embedding = chunk_embedding_cache[resampled]
        denom = np.linalg.norm(removed_embedding) * np.linalg.norm(resampled_embedding)
        if denom:
            return float(np.dot(removed_embedding, resampled_embedding) / denom)
    return lexical_similarity(removed, resampled)
    ...
    for sol in current_solutions:
        ...
        if isinstance(removed, str) and isinstance(resampled, str):
            similarity = chunk_similarity(
                removed, resampled, chunk_embedding_cache
            )
...
def analyze_problem(
    ...
    # Create problem-level embedding cache. If the sentence-transformer cannot be
    # loaded offline, counterfactual metrics fall back to lexical similarity.
    chunk_embedding_cache = {}
    embedding_model = None
    if OFFLINE_ANALYSIS:
        print("Offline analysis mode enabled; using lexical similarity.")
    elif SentenceTransformer is None:
        print("sentence-transformers is unavailable; using lexical similarity.")
    else:
        ...
        try:
            ...
            print(
                f"Could not load sentence model '{sentence_model}' ({exc}); "
    ...
    if embedding_model is not None:
        ...
        for i in tqdm(
            ...
            try:
                ...
                print(f"Embedding failed ({exc}); using lexical similarity.")
    ...
    return {
        "problem_idx": problem_identifier(problem_dir),
...
def generate_plots(
    ...
    for result in results:
        ...
        for chunk in result.get("labeled_chunks", []):
            ...
            # Offline/local analysis may not have LLM labels. Keep those chunks in
            # numeric plots instead of dropping all data and crashing downstream.
            if not formatted_tags:
                formatted_tags = ["Unknown"]
    ...
    if df_chunks.empty:
        print("No chunk-level rows available for plotting; skipping plot generation.")
        return pd.DataFrame()
...
def process_rollouts(
    ...
    if problems:
        requested_problems = {
            item.strip()
            for item in problems.split(",")
            if item.strip()
        }
        problem_dirs = [
            d
            for d in problem_dirs
            if problem_matches_filter(d, requested_problems)
        ]
    ...
    print(
        f"Problems with complete chunk folders: {problems_with_complete_chunks} ({pct(problems_with_complete_chunks, total_problems):.1f}%)"
    )
    print(
        f"Problems with partial chunk folders: {problems_with_partial_chunks} ({pct(problems_with_partial_chunks, total_problems):.1f}%)"
    )
    print(
        f"Problems with no chunk folders: {problems_with_no_chunks} ({pct(problems_with_no_chunks, total_problems):.1f}%)"
    )
