...
There is now a ready-to-edit CHTC workflow under `chtc/` with:

- repo packaging (`chtc/package_repo.sh`)
- Python dependency bundling (`chtc/build_python_bundle.sh`)
- a scratch-safe job wrapper (`chtc/run_repo_job.sh`)
- CPU/GPU HTCondor submit templates (`chtc/submit_cpu.sub`, `chtc/submit_gpu.sub`)

See `chtc/README.md` for the full workflow.
